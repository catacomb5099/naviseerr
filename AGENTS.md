...
3. Process (event-driven) — `DownloadQueue` wraps an in-memory Reactor `Sinks.many().unicast().onBackpressureBuffer()`. `DownloadWorker` subscribes once and processes claimed downloads with bounded concurrency (`download-worker.concurrency`, default 3) via `flatMap`. For each item it runs `DownloadFulfillment.fulfill` (slskd search -> select best files -> enqueue/poll download) and then writes the terminal status via the transactional `DownloadService.markStatus`. Both an error and an empty result map to `FAILED`; every item is isolated so one failure never tears down the worker. The queue sleeps when empty and wakes immediately on emit — no polling on the queue itself (the only interval is the DB claim).
