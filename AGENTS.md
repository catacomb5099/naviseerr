...
The durable state machine described in "Download Manager Architecture" below is now built, not just targeted. Collection downloads landed on 14-09-2026 — see [the ADR](docs/decisions/collection-downloads-14-09-2026.md). Remaining gaps (cancellation, SSE, per-song visibility inside a collection) are called out explicitly below.
...
Two tables live in `com.catacomb5099.naviseerr.download`, and as of V5 the relationship between them is **one download to N tasks** — one task per song:

- `downloads` (`download_id UUID`, `youtube_id TEXT`, `download_type TEXT CHECK (SONG|ALBUM|PLAYLIST)`, `song_name TEXT`, `status TEXT CHECK (...)`, `created_at TIMESTAMPTZ`) — one user REQUEST. The low-churn, user-facing record that history queries read, written exactly twice: at request, and at conclusion. Status values are enforced at the DB level via a `CHECK` constraint rather than a native Postgres enum (see decisions doc for rationale); `PARTIAL_SUCCESS` is in that set and is reachable only for a collection.
  - `song_name` is the download's **display title** — a song's title, or an album/playlist's title — and is nullable, because the request carries only an id. It is written by admission, once the metadata call returns, so it is null for the window between the 202 and the first loop pass.
- `download_tasks` (`task_id UUID PRIMARY KEY`, `download_id UUID REFERENCES downloads`, `youtube_id TEXT`, `song_name TEXT`, `phase`, `phase_entered_at`, `next_attempt_at`, `lease_owner`/`lease_expires_at`, `search_id`, `candidates` as JSON, `candidate_index`, `retry_index`, `slskd_username`/`slskd_filename`/`slskd_transfer_id`, `finished_at`, `failure_reason`, `progress_percent NUMERIC(5,2)`) — the working state of ONE SONG's pipeline, written every few seconds. A song request has one row; a ten-track album has ten. `download_tasks.song_name` is the per-song title that words the Soulseek query, which for a collection is not the download's display title. Rows are **retained** once terminal (`SUCCEEDED`/`FAILED`), never deleted — so a self-hoster can see which peers were tried and how each failed. A partial index on `next_attempt_at` (covering only non-terminal rows) keeps the due-work query fast regardless of how much history accumulates.
...
The client-facing feed is `GET /downloads/active` plus `GET /downloads?ids=` (`DownloadController` + `ActiveDownloadRepository`). It reports **one row per download, never one per song** — its three queries aggregate over the task rows (`ActiveDownloadRepository.TASK_AGGREGATE`), because a plain join emits a ten-track album as ten cards sharing one `downloadId`. For a single-song download every aggregate is over one row and returns that row's own value, so the wire format is unchanged for songs. A collection reports the **least advanced** song's stage (it is still "searching" while any track is) and the **mean** progress across its songs. Four things about the feed are load-bearing and easy to undo by accident:
...
- Per-song visibility inside a collection on the wire. A collection is one card with aggregate progress; which of its tracks failed is in `download_tasks` but is not exposed by any endpoint.
...
- `Download`: **exists**. One user request — a song, an album, or a playlist — identified by a YouTube id and a `DownloadType`.
