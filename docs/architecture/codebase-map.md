...
- `schema/request/` - request-side DTOs consumed rather than returned. `TrackQuery` (song name plus artists; record) is the only member so far - what the download loop carries between passes to word a Soulseek query and score a candidate filename against. It lives here, not in `download`, because `services.slskd` has to consume it (as the input to `SlskdQueryBuilder`) and `download` already depends on `services.slskd`, so putting it in `download` would make that package dependency circular. See [slskd-integration.md](slskd-integration.md).
...
- `services/`
  ...
  - `slskd/` - `SlskdService`, `SlskdConfig`, `SlskdSearchResultProcessor`, `SlskdQueryBuilder` (decides how a `TrackQuery` is worded for Soulseek - primary artist + song name by default, `slskd-service.query-builder.use-all-artists` widens it to every credited artist; added with the song-metadata-table plan). See [slskd-integration.md](slskd-integration.md).
- `download/` - the durable download state machine: `Download`, `Song` (one song's metadata - name, artists, image URL; `@Table("songs")`, written once atomically with its `Download`), `DownloadRequest` (body DTO for `POST /download`: `songName`, `artists`, `imageUrl`), `DownloadStatus`, `DownloadController`, `DownloadService`, `DownloadCandidate`, `DownloadPhase`, `DownloadTask`, `DownloadDecision`, `DownloadStateMachine`, `DownloadTaskRepository`, `DownloadStepExecutor`, `DownloadTaskRunner`. See [download-manager.md](download-manager.md) and [persistence.md](persistence.md).
- `util/`
...
- [DownloadController.java](../../src/main/java/com/catacomb5099/naviseerr/download/DownloadController.java) (`@RestController`):
  - `POST /download` - body `DownloadRequest` (`songName`, `artists`, `imageUrl`); inserts a `PENDING` download and its `songs` row atomically, returns `202 Accepted`; processed asynchronously (see [download-manager.md](download-manager.md)).
  - `POST /download/{songName}` - **deprecated**, kept for one release; delegates to the same insert with empty artists and no image. See [gotchas.md](gotchas.md).
  - `GET /downloads/active`, `GET /downloads?ids=`, `GET /downloads/all` - the download feed, backed by `ActiveDownloadRepository`. See [download-manager.md](download-manager.md) and `docs/decisions/download-progress-reporting-17-08-2026.md`.
