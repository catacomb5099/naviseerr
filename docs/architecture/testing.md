...
- `DownloadServiceClaimIT.java` - **deleted** (14-09-2026), along with the `claimPendingDownloads`/`markStatusIfInProgress` methods it was the only caller of. They had no production caller after the state-machine work; `DownloadTaskRepository.admitDownloads`/`createTasks`/`claimDueTasks` and `DownloadService.finishTask` are what the loop actually uses, and are covered below.
