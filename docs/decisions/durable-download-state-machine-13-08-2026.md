...
## Decision: Batch the two poll phases against slskd; accept an unresolved risk on one of them (revised)
...
**Revision (2026-08-19, from live Task 8 verification):** two claims above were wrong, and both were wrong about `GET /searches` — the endpoint this decision called an unconditional win — not about the transfers endpoint whose risk was the one flagged.

1. **`GET /searches` returns no `responses`.** It has no `includeResponses` parameter and always returns that list empty, whatever the search actually found. So the batched call can report *whether* a search is complete but never *what it found*, and it cannot fully replace the per-search lookup as the plan assumed when it deleted `getSearchResultsProgress`. Selecting candidates off the batched summary therefore yielded zero candidates for every search, failing each download on `NO_CANDIDATES` seconds after its search completed. Symptomatically this presented as "searches got much faster" — the searches were fine; the downloads were dying at the selection gate. **Resolution:** keep the batched call as the completion gate, and refetch the one completed search via `getSearchWithResponses(searchId)` (`GET /searches/{id}?includeResponses=true`) to select from. The batching win is preserved — the refetch is once per download at the completion transition, not once per poll, so call volume stays O(passes) plus O(downloads), never O(in-flight × polls).
2. **Pruning completed searches is dropped.** Beyond being incompatible with needing the results after the completion observation, the delete made `SEARCH_POLL` non-idempotent: a delete that landed while its decision write did not (lost lease, crash between the two) leaves the re-claimed row polling a search slskd no longer has, which the "missing means still running" rule turns into a full `search-budget-ms` stall before failing. Not deleting makes the phase re-runnable, and slskd ages searches out on its own.

3. **`GET /transfers/downloads` is nested, not flat.** Transfers are grouped by peer and then by directory, so reading the response as a flat `Flux<TransferedFile>` parses each top-level *peer* into an all-null transfer, keys the lookup map by `null`, and makes every by-id lookup miss. `TransferedFile` had only ever been validated against the per-transfer endpoint, which does return a bare transfer. **Resolution:** flatten two levels in `getAllDownloads`, pinned by `SlskdServiceTransfersShapeTest` against live-captured JSON, and narrow the map to the transfer ids of the rows claimed in the pass — which also drops null-id entries before they can key the map.
4. **"Missing from the batched map means still running" is withdrawn for transfers.** It stands for searches, where "not there yet" is genuinely indistinguishable from "slskd forgot it". It does not stand for transfers: slskd retains completed transfers (`removed: false`), so absence is a real signal. Aliasing it onto still-running is what let finding (3) run silently for the full hour-long `download-budget-ms` and then report a timeout for a download that had already succeeded. **Resolution:** a distinct not-found branch on a short `missing-transfer-grace-ms` (60s), terminating FAILED with a reason that names the cause — deliberately not SUCCEEDED, since being unable to see a transfer is not evidence that it finished — plus a WARN when tracked ids outnumber matched transfers.

Findings (1) and (3) are the same mistake twice: both batched endpoints were assumed to return the same DTO as their per-item counterpart, and neither does. The lesson recorded for future slskd work is that a batched endpoint's schema must be captured from a live response before it is relied on, not inferred from the single-item endpoint.

The transfers-endpoint *size* risk this decision flagged remains open and unmeasured; the fallback described above still stands unchanged. Also confirmed healthy during the same session: a search state of `"Completed, ResponseLimitReached"` is slskd reporting that the search stopped because it found plenty, and `SlskdSearchState` already classifies it as a non-failure. No recovery path is needed for it.
