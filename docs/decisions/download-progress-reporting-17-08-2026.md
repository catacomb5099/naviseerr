...
- **Status:** implemented (`progress_percent` on `download_tasks`, the `save()` guard, terminal
  normalisation, `GET /downloads/active`). Two items remain open pending a UX test (below); neither
  blocks what landed.
